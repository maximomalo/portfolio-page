...
export default function App() {
  ...
  if (activePage === "shear-perfection") {
...
if (activePage === "now-playing") {
  return <NowPlaying onBack={() => setActivePage(null)} />;
}
  ...
  const caseStudies = [
    ...
    { n: "04", title: "Now Playing", sub: "Steam Player Analytics Dashboard", tag: "Data Visualization", img: "/Analytics.png", page: "now-playing" },
      ...
      <main className="px-12 py-12 space-y-12">
        ...
        <section id="process" className="bg-emerald-600 rounded-md p-10">
          ...
          <div className="space-y-4 text-emerald-100 text-lg leading-relaxed">
            <p>I have experience creating projects using <span className="text-white font-medium">HTML, CSS, and JavaScript</span> — including a completely custom-made replica of an Amazon webpage that involved replicating all of its functionality.</p>
            <p>I am familiar with the basic concepts behind building for the web — structuring websites, styling pages and their various components, and adding functionality through coding.</p>
            <p>Currently, I am enrolled in a <span className="text-white font-medium">UI design class</span> which is providing me with an understanding of fundamental design concepts, basics of user interface, and the way users interact with websites or mobile apps.</p>
            <p>During this semester, I am planning to explore various <span className="text-white font-medium">user-centered design</span> principles and <span className="text-white font-medium">heuristic evaluation</span> techniques by applying these principles and skills in case studies.</p>
          </div>
        </section>
        ...
        <section id="work" className="bg-violet-600 rounded-md p-10">
          ...
          <div className="grid grid-cols-2 gap-4">
            {caseStudies.map(({ n, title, sub, tag, img, page }) => (
              <div
                key={n}
                className="bg-white/10 hover:bg-white/20 transition-colors p-6 rounded-sm cursor-pointer"
              ...
              >
                ...
                {page && <p className="text-violet-300 text-xs mt-2 font-mono">Click to view →</p>}
              </div>
            ))}
          </div>
        </section>
